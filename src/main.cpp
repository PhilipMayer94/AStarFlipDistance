...
int main(int argc, char *argv[]){
    ...
    if(first=="decomposition_runtime_experiments"){

        if(argc>2 &&argc <4){
            do_all_products_for_heuristics_random(argv[2],DECOMPOSITION);
        }
        else{
            do_all_products_for_heuristics_random("../data/random_hod_experiments_paper/",DECOMPOSITION);
        }
    }

    std::cout<<std::endl<<argc<<std::endl;
    if(argc>6||argc<3){
        std::cout<<"to many arguments:\nfirst:  simple eppstein combined heuristic decomposition bfs ilp\nsecond: random sealevel\nthird:  size \nfourth: order/convexorPointset \n fifth: path to data e.g. xyz/abc/data";
        return -22;
    }
    if(argc>5){
